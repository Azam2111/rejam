...
function renderFab(){
  if (state.showAddPlan || state.showAddTask || state.showBackup || state.showCapture
      ...
      || state.showReelImport || state.showReport || state.showSheet || state.showSplit || state.showContentSettings || state.showShootBatch || state.showScriptViewer || state.showExport) return '';
...
// Kartadagi sarlavha 70 belgiga qisqartirilgan - TO'LIQ matn kutubxonadan olinadi.
...
function buildMonthExport(ym, withPrompt){
  ...
  const empty = [];
  for (let i = 1; i <= daysInMonth; i++) if (!covered.has(ym + '-' + pad(i))) empty.push(i);
  return { text: lines.join('\n').trim() + '\n', count: list.length, empty, daysInMonth };
}

// iOS: copyScriptText bilan bir xil tartib - avval eski usul, aynan bosish ichida.
function copyMonthExport(){
  const ex = buildMonthExport(state.exportMonth || defaultExportMonth(), state.exportPrompt);
  const done = () => toast(ex.count + ' kunlik reja nusxalandi');
  if (fallbackCopy(ex.text)) { done(); return; }
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(ex.text).then(done).catch(() => toast('Nusxa olib bo\'lmadi — "Fayl qilib yuborish"ni bosing'));
  } else toast('Nusxa olib bo\'lmadi — "Fayl qilib yuborish"ni bosing');
}

async function shareMonthExport(){
  const ym = state.exportMonth || defaultExportMonth();
  const ex = buildMonthExport(ym, state.exportPrompt);
  const fname = 'rejam-reels-' + ym + '.txt';
  try {
    ...
    <div class="rp-modal-overlay" data-action="close-export">
      <div class="rp-modal rp-modal-tall" data-action="noop">
        ...
        ${!ex.count ? `<div class="rp-empty">${esc(monthLabel(ym))} uchun kunga biriktirilgan reels yo'q.</div>` : `
          ...
          <div class="rp-export-preview">${esc(ex.text)}</div>
...
const handlers = {
  ...
  'export-copy': () => copyMonthExport(),
  'export-share': () => shareMonthExport(),
